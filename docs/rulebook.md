...
1. All inference is local via Ollama and vllm-mlx. No cloud APIs.
2. Provider integrations sit behind a stable, minimal `LlmProvider` interface.
3. The triple-model architecture uses three specialized models: a dense reasoner (planning/review), a MoE coder (tool-calling code generation), and a MoE fast model (triage/delivery). Each runs on its own server instance.
4. Security and redaction logic are centralized in `src/security/*`.
5. Logging goes through the privacy-safe logger (`src/logging/safe-logger.ts`), never direct `console.log` for user data.
6. Configuration is validated at startup via Zod schemas and fails fast on invalid or unsafe settings.
7. Model routing is configured in `config/models.yaml` (Ollama + MLX) and `config/autonomous.yaml` (dual-loop model assignments).
8. The dual-loop is the execution model. FastLoop handles user-facing interaction; DeepLoop handles reasoning and code generation. They communicate only through the TaskBoard.
9. The journal is append-only. Entries are never deleted, only compressed during dream cycles.
10. Delegation is transparent. Every delegated call is logged and reviewable.
...
5. **Update all relevant documentation to reflect the changes.** Any coding change that alters behavior, configuration, architecture, or APIs must be accompanied by corresponding updates to affected docs (architecture, dual-loop, providers, install, configuration, etc.).
6. Run `npm run check` before finishing.
7. If guardrails fail, either revert the risky changes or set `ALLOW_PROTECTED_CHANGES=1` intentionally.
...
3. All relevant documentation is updated to reflect the change.
4. `npm run check` passes locally.
5. Any remaining risk is called out explicitly in the final summary.
