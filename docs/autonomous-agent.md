...
2. Build the agent toolkit with all 74 tools
...
| `src/autonomous/agent-tools.ts` | 74 agent tools: schemas + executors |
...
> **Status:** Toggle removed. 5 dream cycle phases converted to agent tools (`consolidate_reflections`, `reorganize_goals`, `explore_codebase`, `rebuild_self_model`, `write_retrospective`). `runCycle` marked `@deprecated`. Total tools: 74.
