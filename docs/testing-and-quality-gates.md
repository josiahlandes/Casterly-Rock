...
| **Agent** | `agent-loop.test.ts`, `agent-tools.test.ts`, `identity.test.ts`, `journal.test.ts`, `memory-config.test.ts`, `crystal-store.test.ts`, `constitution-store.test.ts`, `trace-replay.test.ts` |
