# Neon Invaders — Coding Capability Test

## Purpose
This prompt tests the DeepLoop's ability to build a complete multi-file project from a detailed specification. The exact same prompt is sent via the REPL each time so results can be compared across code changes (context tiers, manifest system, etc.).
...
## Evaluation Criteria
- **Files created**: Does the structure match the spec?
- **Naming consistency**: Are imports/exports consistent across files?
- **Runs without errors**: Does the game load and start?
- **Feature completeness**: Enemies, shooting, particles, audio, power-ups, combo, levels
- **Visual quality**: Neon glow effects, particle trails, background
